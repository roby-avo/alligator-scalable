...
async def main():    
    ...
    try:
        classifier = Classifier(CLASSIFIER_ENDPOINT, df, input_data["services"]["ClassifiR"], input_data["kg_reference"])
        responses = await classifier.classify_description()
        #print(responses)
        header = list(df.columns)
        for i, item in enumerate(input_data["services"]["ClassifiR"]):
